import type { FillEstimate, Liquidity, MarketSnapshot } from "../types/execution";
...
/** Notional value resting within `bandPercent` of mid price, both sides. Used for the "$ within 1%" metric. */
export function liquidityWithinBand(market: MarketSnapshot, bandPercent = 1): number {
  const bandBid = market.midPrice * (1 - bandPercent / 100);
  const bandAsk = market.midPrice * (1 + bandPercent / 100);
  const bidValue = market.bids
    .filter((l) => l.price >= bandBid)
    .reduce((sum, l) => sum + l.price * l.quantity, 0);
  const askValue = market.asks
    .filter((l) => l.price <= bandAsk)
    .reduce((sum, l) => sum + l.price * l.quantity, 0);
